...
void CAN_cmd_DT7_ch_Date(void)
{
    ...
    DT7_ch_send_data[0] = (rc_ctrl.rc.ch[0] >> 8);
    DT7_ch_send_data[1] = rc_ctrl.rc.ch[0];
    DT7_ch_send_data[2] = (rc_ctrl.rc.ch[1] >> 8);
    DT7_ch_send_data[3] = rc_ctrl.rc.ch[1];
    DT7_ch_send_data[4] = (rc_ctrl.rc.ch[2] >> 8);
    DT7_ch_send_data[5] = rc_ctrl.rc.ch[2];
    DT7_ch_send_data[6] = (rc_ctrl.rc.ch[3] >> 8);
    DT7_ch_send_data[7] = rc_ctrl.rc.ch[3];
...
{
    uint32_t send_mail_box01;
    DT7_tx_message.StdId = DT7_RX_S;
    DT7_tx_message.IDE = CAN_ID_STD;
    DT7_tx_message.RTR = CAN_RTR_DATA;
    DT7_tx_message.DLC = 0x08;
    DT7_ch_send_data[1] = (rc_ctrl.rc.s[0] >> 8);
    DT7_ch_send_data[2] = rc_ctrl.rc.s[0];
    DT7_ch_send_data[3] = (rc_ctrl.rc.s[1] >> 8);
    DT7_ch_send_data[4] = rc_ctrl.rc.s[1];

    HAL_CAN_AddTxMessage(&hcan1, &DT7_tx_message, DT7_s_send_data, &send_mail_box01);
}
